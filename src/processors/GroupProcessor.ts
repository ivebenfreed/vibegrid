...
export class GroupProcessor {
  ...
  static processData(
    ...
    const groupTree = GroupProcessor.buildGroupHierarchy(rows, columns, config, groupRowOrders)

    // Calculate aggregations
    GroupProcessor.calculateAggregations(groupTree.groups, rows, columns, config)

    // Generate virtual rows from group tree
    const virtualRows = GroupProcessor.flattenGroupTree(groupTree.groups, config.expandedGroups)
    ...
    const result: GroupTree = {
      ...
      virtualRows,
      totalHeight,
      groupCount: GroupProcessor.countGroups(groupTree.groups),
    }
  ...
  private static buildGroupHierarchy(
    ...
    if (config.fields.length === 1) {
      return {
        groups: GroupProcessor.buildSingleLevelGroups(rows, columns, config, groupRowOrders),
      }
    } else {
      return { groups: GroupProcessor.buildMultiLevelGroups(rows, columns, config, groupRowOrders) }
    }
  }
  ...
  private static buildSingleLevelGroups(
    ...
    if (column && column.options && Array.isArray(column.options) && column.options.length > 0) {
      column.options.forEach((option) => {
        ...
        const groupKey = GroupProcessor.getGroupKey(optionValue)
    ...
    rows.forEach((row, index) => {
      ...
      const groupKey = GroupProcessor.getGroupKey(value)
    ...
    groupMap.forEach((groupRows, groupKey) => {
      ...
      if (groupRows.length === 0) {
        ...
        value = GroupProcessor.getValueFromGroupKey(groupKey)
      ...
      if (groupRows.length > 0 && groupRowOrders && groupRowOrders[groupId]) {
        orderedChildren = GroupProcessor.applyCustomRowOrdering(groupRows, groupRowOrders[groupId])
    ...
    // Sort groups
    GroupProcessor.sortGroups(groups, config)
    ...
    const buildLevel = (
      ...
      remainingRows.forEach((row) => {
        ...
        if (row && row.data) {
          ...
          fileLog.warn('GroupProcessor: Skipping invalid row in multi-level grouping', {
            row,
            fieldName,
          })
          return
      ...
      groupMap.forEach((groupRows, groupKey) => {
        ...
        if (firstRow && firstRow.data) {
          ...
          fileLog.warn('GroupProcessor: Invalid firstRow in multi-level groupMap', {
            firstRow,
            fieldName,
          })
          return
        ...
        const groupNode: GroupNode = {
          ...
          sortOrder: sortOrder++,
          aggregations: [],
          children: isLastLevel ? groupRows : buildLevel(groupRows, fieldIndex + 1, groupId),
        }

        // Calculate totalCount for nested groups
        if (!isLastLevel) {
          groupNode.totalCount = GroupProcessor.calculateTotalCount(groupNode)
        } else {
  ...
  private static calculateAggregations(
    ...
    const calculateForGroup = (group: GroupNode): void => {
      ...
      const groupRows = GroupProcessor.getGroupDataRows(group)
      ...
      config.aggregations.forEach((aggConfig) => {
        ...
        switch (aggConfig.function) {
          ...
          case 'sum':
            ...
            displayValue = GroupProcessor.formatNumber(result as number)
            break

          case 'avg': {
            const sum = fieldValues.reduce((sum, val) => sum + (Number(val) || 0), 0)
            result = sum / fieldValues.length
            displayValue = GroupProcessor.formatNumber(result, 2)
            break
          }

          case 'min':
            result = Math.min(...fieldValues.map((v) => Number(v) || 0))
            displayValue = GroupProcessor.formatNumber(result)
            break
  ...
  static flattenGroupTree(groups: GroupNode[], expandedGroups: Set<string>): VirtualRow[] {
  ...
  private static calculateTotalCount(group: GroupNode): number {
    if (Array.isArray(group.children) && group.children.length > 0) {
      return group.children.reduce((total, child) => {
        if ('field' in child) {
          // Child is a group
          return total + GroupProcessor.calculateTotalCount(child as GroupNode)
        } else {
          // Child is a data row
          return total + 1
        }
  ...
  private static countGroups(groups: GroupNode[]): number {
    ...
    groups.forEach((group) => {
      if (Array.isArray(group.children)) {
        const childGroups = group.children.filter((child) => 'field' in child) as GroupNode[]
        count += GroupProcessor.countGroups(childGroups)
      }
    })
