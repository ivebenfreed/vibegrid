  ...
  const tableInteraction$ = observable({
    ...
    getRowCheckboxStates(rows: any[], visibleColumns: any[]): Map<string, boolean> {
      ...
      for (const row of rows) {
        const isRowSelected =
          dataColumns.every((col) => selectedCells.has(`${row.id}:${col.id}`)) &&
    ...
    selectCell(cellId: string, isMulti: boolean = false) {
      ...
      fileLog.info('<� Cell selected', {
        cellId,
        isMulti,
        selectionCount: tableInteraction$.selectedCells.get().size,
      })
    },
    ...
    handleCellClick(cellId: string, isEditable: boolean, ctrlKey: boolean, shiftKey: boolean) {
      ...
      if (now - lastBulkSelection < 50) {
        // Within 50ms of bulk selection
    ...
    selectRow(rowId: string, isMulti: boolean = false) {
      ...
      fileLog.info('<� Row selected', {
        rowId,
        isMulti,
        selectionCount: tableInteraction$.selectedRows.get().size,
      })
    },
    ...
    async saveEdit(finalValue?: any) {
      ...
      if (!entityType) {
        fileLog.warn('⚠ No entity type available for entity update', {
          editingCell,
          rowId,
          fieldName,
        })
