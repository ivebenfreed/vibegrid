...
import { batch, computed, observable, when } from '@legendapp/state'
...
function computeVisualRows(inputs: any): VirtualRow[] {
  ...
  fileLog.debug(
    '🔄 Transition state: grouping configured but processedRows in raw format, converting',
    {
      groupConfig,
...
export function createVisualOperations(visualInputs$: any, visualState$: any) {
  ...
  return {
    ...
    setColumnWidth(columnId: string, width: number) {
      ...
      fileLog.info(
        '[RESIZE] 📏 Column width updated in visualInputs$ - should trigger visual observer',
        {
          columnId,
          width,
    ...
    handleViewportScroll(
      ...
      if (currentScrollLeft === scrollLeft && currentScrollTop === scrollTop) {
        fileLog.debug('📜 Scroll event with same values - skipping update', {
          scrollLeft,
          scrollTop,
          source,
        })
    ...
    isColumnInViewport(columnId: string): boolean {
      ...
      return (
        column.xOffset < scrollLeft + viewportWidth && column.xOffset + column.width > scrollLeft
      )
    },
    ...
    toggleGroupExpansion(groupId: string) {
      ...
      fileLog.info('🔍 toggleGroupExpansion called', {
        groupId,
        hasGroupConfig: !!groupConfig,
        currentExpandedGroups: groupConfig?.expandedGroups
          ? Array.from(groupConfig.expandedGroups)
          : 'none',
      })
      ...
      if (wasExpanded) {
        ...
        fileLog.info('🎯 Group collapsed', {
          groupId,
          newExpandedGroups: Array.from(expandedGroups),
        })
      } else {
        expandedGroups.add(groupId)
        fileLog.info('🎯 Group expanded', {
          groupId,
          newExpandedGroups: Array.from(expandedGroups),
        })
      }
      ...
      fileLog.info('✅ Group config updated', {
        ...
        finalExpandedGroups: updatedConfig?.expandedGroups
          ? Array.from(updatedConfig.expandedGroups)
          : 'none',
      })
    },
     ...
     */
    initializeColumns(columns: Column[], entityType: string, orgId: string, userId: string) {
     ...
     */
    createDefaultColumnState(columns: Column[], entityType: string, orgId: string, userId: string) {
      ...
      const finalColumnOrder =
        savedPrefs.columnOrder && savedPrefs.columnOrder.length > 0
      ...
      return {
        ...
        columnVisibility:
          savedPrefs.columnVisibility || Object.fromEntries(columns.map((col) => [col.id, true])),
    ...
    showAllColumns() {
      const columns = visualInputs$.columns.get()
      const allVisible = Object.fromEntries(columns.map((col) => [col.id, true]))
    ...
    toggleSort(field: string, isMultiSort: boolean = false) {
      try {
        ...
        fileLog.info('🔄 Sort toggled - ENHANCED DEBUG', {
          ...
          sortByLength: Array.isArray(currentSortAfterToggle)
            ? currentSortAfterToggle.length
            : 'not-array',
    ...
    try {
      ...
      if (previousState) {
        ...
        hasDataChanges =
          ...
          JSON.stringify(sortBy.map((s) => s.id)) !==
            JSON.stringify(previousState.data.sortBy.map((s) => s.id))
      ...
      const renderState = {
        ...
        shouldUpdateOverlays:
          hasInteractionChanges || isDragging || editingCell !== null || columnResize !== null,
        hasDataChanges,
        hasLayoutChanges,
        hasInteractionChanges,
      }
