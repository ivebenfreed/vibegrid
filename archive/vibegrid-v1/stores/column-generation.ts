...
import { universeLoading$, universeSchema$ } from '@/legend-state/observables'
...
export async function generateColumnsFromEntitySchema<T = any>(
  ...
  if (!entitySchema) {
    ...
    throw new Error(
      `Entity ${entityType} not found in universe schema. Available: ${Object.keys(universeSchema.entities).join(', ')}`,
    )
  }
  ...
  const allColumns = schemaFields
    .map((fieldDef: any) => {
      ...
      const safeFieldDef: EntityField =
        fieldDef && typeof fieldDef === 'object' ? fieldDef : { name: fieldName, type: 'text' }
