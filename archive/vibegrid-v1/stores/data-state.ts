...
export function createTableCore$(entityType: string, visualInputs$?: any, initManager?: any) {
  fileLog.info('🎯 Creating tableCore$ observable (schema loading managed by init manager)', {
    entityType,
  })
  ...
  const tableCore$ = observable({
    ...
    get processedRows() {
      ...
      fileLog.info('📊 Group config from visual state', {
        ...
        expandedGroups: groupConfig?.expandedGroups
  ...
  if (orgId && userId) {
    ...
    fileLog.warn('� Cannot initialize columns observable - missing orgId or userId', {
      orgId,
      userId,
    })
  }
  ...
  fileLog.info('✅ TableCore$ created, waiting for init manager to trigger schema loading', {
    entityType,
  })
