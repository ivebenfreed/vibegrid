...
export class BodyRenderer {
  ...
  createCellElement(row: any, column: any, colIndex: number, xPosition?: number): HTMLElement {
    ...
    if (this.modularCellBridge) {
      try {
        ...
        const cellElement = this.modularCellBridge.createCell(value, column, rowData, {
          rowIndex: 0,
  ...
  updateAllRowCheckboxes(): void {
    ...
    const checkboxStates = this.tableInteraction$.getRowCheckboxStates(
      processedRows,
      allVisibleColumns,
    )
  ...
  private initializeDragDrop(): void {
    this.dragDropManager = new DragDropManager({
      onRowMove: (draggedRowId: string, targetGroupId: string, newIndex: number) => {
        ...
        const success = this.tableCore$.moveRowInGroup(
          sourceGroupId,
          targetGroupId,
          draggedRowId,
          newIndex,
