...
export class SimplePassiveRenderer {
  ...
  constructor(private options: SimplePassiveRendererOptions) {
    ...
    fileLog.debug(
      '🎯 DEFERRED: Skipping initFocusedObservers during construction - will attach after initialization',
    )
  ...
  private initFocusedObservers(): void {
    ...
    fileLog.debug(
      '🎯 Creating sorted data observer - direct subscription to sortedProcessedRows$ changes',
    )

    if (!this.sortedProcessedRows$) {
      fileLog.error(
        '❌ DATA FLOW ERROR: sortedProcessedRows$ not available - cannot set up observer',
      )
      return
    }
    ...
    this.columnVisibilityObserverDisposer =
      this.visualState.visualInputs$.columnVisibility.onChange(() => {
        // GUARD: Skip if observers are not enabled yet
        if (!this.observersEnabled) {
    ...
    this.interactionObserverDisposer = observe(() => {
      ...
      if (
        currentSelectAllChecked !== this.lastSelectAllChecked ||
      ...
      if (
        currentColumnResize?.isResizing &&
        currentColumnResize.columnId &&
        currentColumnResize.newWidth
      ) {
        ...
        this.columnWidthManager?.updateHeaderCellWidth(
          currentColumnResize.columnId,
          currentColumnResize.newWidth,
        )
        this.columnWidthManager?.updateBodyCellWidths(
          currentColumnResize.columnId,
          currentColumnResize.newWidth,
        )
      }
    ...
    this.scrollObserverDisposer = observe(() => {
      ...
      if (
        this.initManager.hydrationState$.rendererInitialized.get(true) &&
    ...
    this.virtualScrollObserverDisposer = this.visualState.visualInputs$.scrollTop.onChange(() => {
      ...
      const endRowIndex = Math.min(
        rowCount,
        Math.ceil((newScrollTop + Math.max(viewportHeight, 400)) / rowHeight) + 1,
      )
        ...
        requestAnimationFrame(() => {
          ...
          requestAnimationFrame(() => {
            ...
            batch(() => {
              ...
              this.lastVisibleColumns =
                this.visualState.visualState$.geometry.visibleColumnRange.get()
  ...
  private updateVirtualRows(
    ...
    const rows = this.sortedProcessedRows$
      ? this.sortedProcessedRows$.get(true)
      : this.tableCore$.processedRows.get(true)
    ...
    if (previousRange.start === -1) {
      ...
      for (let i = currentRange.start; i <= currentRange.end && i < rows.length; i++) {
        const rowElement = this.bodyRenderer.createRowElement(
          rows[i],
          i,
          columns,
          columnVisibility,
  ...
  private handleConsolidatedVisualStateChange(visualState: VisualState): void {
    ...
    batch(() => {
      ...
      const needsHeaderRender =
        ...
        JSON.stringify(previousState.columns.map((c) => c.width)) !==
          JSON.stringify(visualState.columns.map((c) => c.width))
