          ...
          if (clipboardChanged) {
            if (state.clipboard && state.clipboard.copiedCells.size > 0 && this.canvasOverlay) {
              ...
              const clipboardVisualCells = this.getVisualCellPositions(
                Array.from(state.clipboard.copiedCells),
              )
    ...
    if (cell) {
      ...
      fileLog.debug('🔍 DIAGNOSTIC: Container debug info', {
        ...
        scrollingContext: {
          ...
          shouldUseViewportForCalculation:
            !!viewportContainer && viewportContainer !== this.container,
        },
      })
      ...
      if (viewportContainer) {
        ...
        fileLog.debug('✅ DIAGNOSTIC: Using direct DOM calculation with scroll analysis', {
          ...
          calculation: {
            ...
            cellVisibleWidth:
              Math.min(cellRect.right, viewportRect.right) -
              Math.max(cellRect.left, viewportRect.left),
            isPartiallyVisible:
              cellRect.right > viewportRect.right || cellRect.left < viewportRect.left,
            isFullyVisible:
              cellRect.left >= viewportRect.left && cellRect.right <= viewportRect.right,
          },
          scrollDiagnostic: {
            scrollLeft: currentScrollLeft,
            ...
            isCellOutsideViewport:
              cellRect.right < viewportRect.left || cellRect.left > viewportRect.right,
          },
        })
