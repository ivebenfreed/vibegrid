...
export class MouseController {
  ...
  private onMouseMove(e: MouseEvent): void {
    ...
    if (
      ...
      !this.isColumnDrag &&
      !this.isRowDrag &&
      this.tableInteraction$.isDragSelecting.get()
    ) {
      const target = e.target as HTMLElement
      const cellElement = target.closest('[data-row-id][data-column-id]')
      ...
      if (cellElement) {
        ...
        if (currentCellId !== currentDragCell) {
          ...
          try {
            ...
            const rows =
              this.visualState.virtualizedData ||
    ...
    if (this.isDragging || this.isColumnResize) {
      if (this.isColumnResize) {
        ...
        if (targetColumnId && targetColumnId !== this.dragColumnId) {
          ...
          this.visualState.visualOperations.reorderColumns(
            this.dragColumnId,
            targetColumnId,
            insertBefore,
          )
      ...
      if (cellElement && this.selectionController) {
        ...
        if (e.shiftKey) {
          ...
          if (lastSelectedCell) {
            ...
            this.tableInteraction$.selectRange(lastSelectedCell, cellId, {
              rows,
              columns,
              columnVisibility,
            })
