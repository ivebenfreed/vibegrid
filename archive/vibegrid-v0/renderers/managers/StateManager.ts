...
import type { Column, RendererOptions, RenderState, SortConfig, TableRow } from '../../types'
...
export class StateManager {
  ...
  setColumns(columns: Column[]): void {
    fileLog.warn(
      'StateManager: setColumns is deprecated - column data comes from state machine coordinate mapping',
    )
    // Column information now comes from state machine coordinate mapping
  }
  ...
  setColumnVisibility(visibility: Record<string, boolean>): void {
    fileLog.warn(
      'StateManager: setColumnVisibility is deprecated - column visibility comes from state machine coordinate mapping',
    )
  ...
  setColumnOrder(order: string[]): void {
    fileLog.warn(
      'StateManager: setColumnOrder is deprecated - column order comes from state machine coordinate mapping',
    )
  ...
  private getCellElement(rowId: string, columnId: string): HTMLElement | null {
    return this.config.domManager
      .getElement('body')
      .querySelector(`[data-row-id="${rowId}"][data-column-id="${columnId}"]`) as HTMLElement
  }
