...
export function VibeGrid<T extends Record<string, any> = any>(
  ...
  return (
    ...
    >
      {/* Status indicator */}
      {error && (
        <div className="p-2 mb-2 bg-red-100 text-red-700 rounded text-sm">Error: {error}</div>
      )}
      ...
      {process.env.NODE_ENV === 'development' && isInitialized && (
        <div className="p-2 border-t bg-muted/50 text-xs space-y-1">
          ...
          <p>
            <strong>Renderer:</strong> PassiveTableRenderer with granular observers
          </p>
          <p>
            <strong>Event Binding:</strong> Direct DOM → Observable methods
          </p>
          <p>
            <strong>XState:</strong> ❌ Removed (no state machines)
          </p>
        </div>
      )}
    </div>
