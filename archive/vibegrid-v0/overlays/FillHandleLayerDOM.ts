...
export class FillHandleLayerDOM {
  ...
  constructor(container: HTMLElement, config: FillHandleConfig, callbacks: FillHandleCallbacks) {
  ...
  calculateFillPreviewCells(
    ...
    for (let i = 1; i <= rowsToFill; i++) {
      const targetRowIndex = fillDown ? bounds.maxRowIndex + i : bounds.minRowIndex - i
