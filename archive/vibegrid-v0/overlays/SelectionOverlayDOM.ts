...
export class SelectionOverlayDOM {
  ...
  updateSelectionWithMapping(
    ...
    if (!viewport || !coordinateMapping) {
      myLog.warn(
        'SelectionOverlayDOM: Missing viewport or coordinate mapping for selection render',
        {
  ...
  updateWithVisualPositions(visualCells: VisualCellPosition[]): void {
    ...
    for (const cell of visualCells) {
      ...
      myLog.info(
        `SelectionOverlayDOM: ${isNewSelection ? 'Adding new' : 'Updating existing'} element for ${cellKey}`,
        {
          x: cell.x,
          y: cell.y,
          width: cell.width,
  ...
  private addOrUpdateSelectionElement(
    ...
    if (isPositionChange && element.style.opacity !== '0') {
      ...
      element.style.transition =
        'left 200ms ease-out, top 200ms ease-out, width 200ms ease-out, height 200ms ease-out, opacity 200ms ease-out, transform 200ms ease-out'
  ...
  updateSelectionRange(
    ...
    const startColIdx = coordinateMapping.columns.findIndex(
      (c) => c.columnId === startCell.columnId,
    )
